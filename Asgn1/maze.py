        ...
        while True:
            ...
            for action, state in self.neighbors(node.state):
                ...
                if state not in self.explored and not frontier.contains_state(state):
                    ...
                    # Priority  - need to add the appropriate code
    ...
    def output_image(self, filename, show_solution=True, show_explored=False):
        ...
        for i, row in enumerate(self.walls):
            for j, col in enumerate(row):
                ...
                # Draw cell
                draw.rectangle(
                    (((float(j * cell_size + cell_border), float(i * cell_size + cell_border)),
                      (float((j + 1) * cell_size - cell_border), float((i + 1) * cell_size - cell_border)))),
                    fill=fill
                )
